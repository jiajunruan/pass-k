...
def process_query_responses(query_data):
    ...
    for i in range(1, 201):  # i = 1..200
        ...
        prefix_es.append(running_sum / i)     # 前 i 个 ES 的平均值
    # running_sum = responses[0]["ES"]
    # 返回 200 维 prefix ES 数组
    return running_sum
    ...
    with open(input_file_path, 'r', encoding='utf-8') as infile:
        for line in tqdm(infile, desc=f"Processing {os.path.basename(input_file_path)}"):
            try:
                ...
                prefix_es = process_query_responses(query_data)
